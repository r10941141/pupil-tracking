        ...
        for action_number in range(26):
            ...
            for idx in tqdm(range(nr_image), desc=f'[{sequence_idx:03d}] {image_folder}'):
                ...
                if 'model' not in locals():
                    ...
                    model_path = './outputs/datafile_16/ML_for_pupil.hdf5'
                    model = load_model(model_path, compile=False)
                    from postprocessing.data_postprocessing import postprocessing_and_confidence

                input_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                input_resized = cv2.resize(input_gray, (512, 512), interpolation=cv2.INTER_LINEAR)
                input_tensor = np.expand_dims(input_resized.astype(np.float32) / 10000.0, axis=(0, -1))

                pred_mask = model.predict(input_tensor)[0, :, :, 0]
                pred_mask_bin = (pred_mask > 0.5).astype(np.uint8) * 255
                restored_size = cv2.resize(pred_mask_bin, (image.shape[1], image.shape[0]))

                output, conf = postprocessing_and_confidence(restored_size)
...
if __name__ == '__main__':
    dataset_path = r'.\data\public\S1\01' 
